...
'  Loop
  ...
  '
  ReDim Recognized_Idx_iModule(0 To 3) As Integer
  ReDim Recognized_Z_PROGRAMKEY(0 To 3) As String
  ...
  Recognized_Idx_iModule(3) = 20
  Recognized_Z_PROGRAMKEY(3) = "MFB"
  ''''Recognized_Idx_iModule(4) = 21
  ''''Recognized_Z_PROGRAMKEY(4) = "MFB"
  '
  ...
  For i = LBound(Recognized_Idx_iModule) To UBound(Recognized_Idx_iModule)
    ...
    If (snumIsModulePurchased(ThisSerialNumber, Recognized_Idx_iModule(i)) = 1) Then
      num_pkdt_new = num_pkdt_new + 1
      ReDim Preserve pkdt_new(1 To num_pkdt_new)
...
Dim Revised_Z_EXPIRATIONDATE As String
  ...
  End If
If (DebugMode) Then Call Debug_Output("a1") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  '
  ...
  End If
If (DebugMode) Then Call Debug_Output("a2") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  Call LicFile_GenerateEncryptionKey    'must be called once before any other LicFile_*() routine.
  ...
  If (Not found_it) Then
    ...
    'Call ExitError(arg_CpasDir, LICFILE_BadLicenseFile)
  End If
If (DebugMode) Then Call Debug_Output("a3") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  '
  ...
  If (FileExists(fn_MTCHKLIC)) Then
    ...
    If (Trim$(UCase$(dummy)) <> Trim$(UCase$(LICFILE_ExtraCheckFile_Text))) Then
      'EXIT WITH AN ERROR: CREATE {CPAS}\DBASE\EXIT.X.
If (DebugMode) Then Call Debug_Output("a4") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
      Call ExitError(fpath_OutputDir, LICFILE_BadLicenseFile)
      'Call ExitError(arg_CpasDir, LICFILE_BadLicenseFile)
    End If
  ...
  Else
    ...
    On Error GoTo err_Do_Get_Info_v11
  End If
If (DebugMode) Then Call Debug_Output("a5") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  '
  'VERIFY THE SERIAL NUMBER.
  '
  ''''result = Snums_Verify(Trim$(lfdt.Z_SERIALNUMBER))
  result = snumVerify(Trim$(lfdt.Z_SERIALNUMBER))
If (DebugMode) Then Call Debug_Output("A5A") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  If (result = 0) Then
If (DebugMode) Then Call Debug_Output("A5B") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    'EXIT WITH AN ERROR: CREATE {CPAS}\DBASE\EXIT.X.
...
If (DebugMode) Then Call Debug_Output("A5D") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    Call ShowError(msg)
If (DebugMode) Then Call Debug_Output("A5E") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    GoTo exit_err_Do_Get_Info_v11
  Else
If (DebugMode) Then Call Debug_Output("A5C") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    'THAT'S OKAY.
  End If
If (DebugMode) Then Call Debug_Output("a6") ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  '
  ...
  Select Case copy_z_versiontype
    ...
    Case Else:      'ASSUME EXTERNAL_WILL_EXPIRE.
      ...
      Revised_Z_EXPIRATIONDATE = Trim$(UCase$(pkdt.Z_EXPIRATIONDATE))
      For i = 1 To Len(Revised_Z_EXPIRATIONDATE)
        If (Mid$(Revised_Z_EXPIRATIONDATE, i, 1) = "/") Then
          Mid$(Revised_Z_EXPIRATIONDATE, i, 1) = ","
        End If
      Next i
      Revised_Z_EXPIRATIONDATE = Parser_RemoveCharacters(" ", Revised_Z_EXPIRATIONDATE)
      copy_z_expirationdate = Revised_Z_EXPIRATIONDATE
...
'MsgBox "fn_CPASLIC = `" & fn_CPASLIC & "`"
  ...
  '
Dim ThisIntSnum As Long
  ThisIntSnum = snumGetInternalSnum(Trim$(lfdt.Z_SERIALNUMBER))
  If (ThisIntSnum = 0) Then
    'EXIT WITH AN ERROR: CREATE {CPAS}\DBASE\EXIT.X.
    msg = "The serial number for the software is invalid.  "
    msg = msg & "Please contact "
    msg = msg & LICFILE_ExpireContactInfo & " to correct "
  ...
  Else
    'THAT'S OKAY.
  End If
  lfdt.Z_USERCODE = ThisIntSnum     'Trim$(Str$(ThisIntSnum))
  '
  ...
  Revised_Z_EXPIRATIONDATE = pkdt.Z_EXPIRATIONDATE
  For i = 1 To Len(Revised_Z_EXPIRATIONDATE)
    If (Mid$(Revised_Z_EXPIRATIONDATE, i, 1) = "/") Then
      Mid$(Revised_Z_EXPIRATIONDATE, i, 1) = ","
    End If
  Next i
  Print #f, Revised_Z_EXPIRATIONDATE
  Print #f, pkdt.Z_RELEASETYPE
  Print #f, pkdt.Z_VERSIONCODE
  Print #f, pkdt.Z_VERSIONTYPE
  Print #f, Trim$(Str$(lfdt.Z_USERCODE))
  Close #f
  'MsgBox "got here (9)"
  End
