...
Begin VB.Form frmD7_SecondaryWeir 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Enter Parameters [Secondary Clarifier Weir]"
   ClientHeight    =   4680
   ClientLeft      =   1980
   ClientTop       =   3465
   ClientWidth     =   7500
   ...
   Begin VB.Label Label1 
      Caption         =   $"D7_SecondaryWeir.frx":0000
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmD7_SecondaryWeir"
...
Dim frmD7_SecondaryWeir_Is_Dirty As Boolean
...
Const frmD7_SecondaryWeir_declarations_end = True


Sub frmD7_SecondaryWeir_Edit( _
    OUTPUT_Raise_Dirty_Flag As Boolean)
  Temp_Plant = NowProj.Plant
  frmD7_SecondaryWeir.Show 1
  If (USER_HIT_OK) Then
...
Sub frmD7_SecondaryWeir_PopulateUnits()
Dim Frm As Form
Set Frm = frmD7_SecondaryWeir
  ...
  Call unitsys_register(Frm, lblData(0), txtData(0), cboUnits(0), "length", _
      Temp_Plant.SecondaryWeir.UnitsOfDisplay(0), "m", "", "", 100#, True)
  Call unitsys_register(Frm, lblData(1), txtData(1), cboUnits(1), "length", _
      Temp_Plant.SecondaryWeir.UnitsOfDisplay(1), "m", "", "", 100#, True)
  ''''Call unitsys_register(Frm, lblData(2), txtData(2), cboUnits(2), "flow_volumetric_per_length", _
      Temp_Plant.SecondaryWeir.UnitsOfDisplay(2), "m³/m-h", "", "", 100#, True)
  Call unitsys_register(Frm, lblData(2), txtData(2), cboUnits(2), "flow_volumetric_per_length", _
      Temp_Plant.SecondaryWeir.UnitsOfDisplay(2), "m³/(m-h)", "", "", 100#, True)
End Sub
...
Set Ctl = cbo_Model_Type
  ...
  Temp_Plant.SecondaryWeir.ModelingMechanism = Ctl.ItemData(Ctl.ListIndex)
  'RAISE DIRTY FLAG AND REFRESH WINDOW.
  Call Local_DirtyStatus_Set(frmD7_SecondaryWeir_Is_Dirty, True)
  Call frmD7_SecondaryWeir_Refresh(Temp_Plant)
End Sub
...
Private Sub Form_Load()
  ...
  Call Local_DirtyStatus_Set(frmD7_SecondaryWeir_Is_Dirty, False)
  ...
  '
  Call frmD7_SecondaryWeir_PopulateUnits
  '
  ' REFRESH DISPLAY.
  '
  Call frmD7_SecondaryWeir_Refresh(Temp_Plant)
  ...
  If (NewValue_Okay) Then
    If (Raise_Dirty_Flag) Then
      ...
      Select Case Index
        ...
        ' MAIN DATA BLOCK.
        '
        Case 0: Temp_Plant.SecondaryWeir.Width = NewValue
        Case 1: Temp_Plant.SecondaryWeir.WaterLevelDiff = NewValue
        Case 2: Temp_Plant.SecondaryWeir.GasFlow = NewValue
      End Select
      If (Raise_Dirty_Flag) Then
        'THROW DIRTY FLAG.
        Call Local_DirtyStatus_Set(frmD7_SecondaryWeir_Is_Dirty, True)
