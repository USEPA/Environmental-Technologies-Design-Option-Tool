...
'            fn_OutputTxt = App.Path + "\exes\output.txt"
            ...
            Dim end_chr As Integer
            Dim text_left
            Dim input_text As String
            Dim length_left As Integer
            Dim text_length As Integer
            
            Open fn_outputtxt For Output As #f
            text_left = Prj.dyestudy_output
            text_length = Len(text_left)
            Do While text_length > 0
              end_chr = InStr(text_left, ",")
              If end_chr = 0 Then
                input_text = text_left
                Print #f, input_text
                Exit Do
              End If
              input_text = left$(text_left, end_chr - 1)
              If input_text <> "," Then
                 Print #f, input_text
              End If
              text_length = text_length - end_chr
              text_left = Right$(text_left, text_length)
              Loop
            Close #f
