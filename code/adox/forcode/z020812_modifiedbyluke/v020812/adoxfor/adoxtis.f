...
C	MODIFIED BY KELI ON 02/06/27 FOR ALK<->tic CONVERSION
	DOUBLE PRECISION BICARBN
C	MODIFY BY KELI ENDS
...
C
C------ CODE ADDED BY ERIC J. OMAN ON 7/2/98 BEGINS.
C
      CHARACTER*80 FN_PH_OUTPUT
C
C------ CODE ADDED BY ERIC J. OMAN ON 7/2/98 ENDS.
C
C
      NFLAG=0
C
...
C
      DO I = 1, NREV
C 
C--- Ke Li revise begin
C	      XKE(I)=10**(-XKE(I))
	  IF (XKE(I) .GE. 100) THEN
	      XKE(I) = 0
	  ELSE
	      XKE(I)=10**(-XKE(I))
	  ENDIF
...
C 
C	MODIFIED BY KELI ON 02/06/27 FOR ALK<->tic CONVERSION
		BICARBN = (ALK/5E4+10**(-PH(1,0))-10**(PH(1,0)-14))
     &               /(1+2*10**(PH(1,0)-10.3))
		TICARBN = BICARBN*(1+10**(6.3-PH(1,0))+10**(PH(1,0)-10.3))
C	MODIFY BY KELI ENDS
C --- Ke Li revise begin
C        TICARBN = ALK/50.0
C        TICARBN is in mol/l 
...
C
C
C------ CODE MODIFIED BY ERIC J. OMAN ON 7/2/98 BEGINS.
C
	DO I = 1, NODE
	...
	ENDDO
C
C------ CODE MODIFIED BY ERIC J. OMAN ON 7/2/98 ENDS.
C
C
...
C------ CODE ADDED BY ERIC J. OMAN ON 7/2/98 BEGINS.
C
      OPEN(7,FILE=FN_PH_OUTPUT,STATUS='UNKNOWN')
      WRITE(*,*) '- Writing to ', FN_PH_OUTPUT
      WRITE(7,*) 'pH'
      WRITE(7,*) 'dimensionless'
	WRITE(7,150)
	DO NT = 1, NTANK
        DO J = 0, NSTEPS,OPSIZE*60/SSIZE
	    WRITE(7,200) NT,TIME(J),TIMEMIN(J),CONC(N+NT,J)
...
151   FORMAT('#TANK',T7,'TIME(SEC)',T17,'TIME(MIN)',T27,'CONC')
