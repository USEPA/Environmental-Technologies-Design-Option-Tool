...
C ----Changed by Ke Li
C	DO I = 1, NODE
C	  YDOT(I)=YDOT(I)+IDREACT*NTANK*(CONCINI(I)-YO(I))/TAU
C	ENDDO
C	DO NT = 2, NTANK 
C	  NITANK = (NT-1)*NODE
C	  DO I = NITANK+1, NITANK+NODE
C         YDOT(I)=YDOT(I)+IDREACT*NTANK*(YO(I-NODE)-YO(I))/TAU
C	  ENDDO
C     ENDDO

C     The TAU is the hydraulic retention time of a single tank
	DO I = 1, NODE
	  YDOT(I)=YDOT(I)+IDREACT*(CONCINI(I)-YO(I))/TAU
	ENDDO
	DO NT = 2, NTANK 
	  NITANK = (NT-1)*NODE
	  DO I = NITANK+1, NITANK+NODE
          YDOT(I)=YDOT(I)+IDREACT*(YO(I-NODE)-YO(I))/TAU
	  ENDDO
      ENDDO
C
C------ Ke Li change end.
