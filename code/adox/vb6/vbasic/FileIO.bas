...
Dim DataVersion_Minor As Integer
  ...
  On Error Resume Next
' It was decided not to password protect the example files
'  If UCase(get_program_releasetype()) = "BETA" Then
'    Set DbTest1 = OpenDatabase(fn_open, _
'                True, _
'                False, _
...
'  End If
  ...
  If (ShowLegacyWarning) Then
    Call Show_Message("Warning: This file is formatted as a " & _
...
'    If UCase(get_program_releasetype()) = "BETA" Then
'      frmMain.CommonDialog1.Filter = "All Files (*.*)|*.*|" & _
'        Name_App_Short & " Files (*." & BetaFileExt_app & ")|*." & _
'        BetaFileExt_app
'    Else
      frmMain.CommonDialog1.Filter = "All Files (*.*)|*.*|" & _
...
Dim RetVal As Integer
Dim msg As String
  If (Not Project_IsDirtyFlagThrown()) Or _
    UCase(get_program_releasetype()) = "BETA" Then
