...
Dim ut As String
Dim un As String
Dim ColumnArea As Double
Dim Valence As Double
Dim MolecularWeight As Double
  ...
  If (ut = "TIME") Then
    If (un = "MIN") Then x = 1#
    If (un = "S") Then x = 1# * 60#
    If (un = "HR") Then x = 1# / 60#
    If (un = "D") Then x = 1# / 1440#
'    If (un = "YEAR") Then x = 1# / 60# / 24# / 365.25
  End If
  ...
  If (ut = "CONCENTRATION") Then
    Select Case Trim$(frmAddComponent.Caption)
        Case "Edit Anion"
            MolecularWeight = NowProj.Anion(NumberOfIonToEdit).MolecularWeight
            Valence = NowProj.Anion(NumberOfIonToEdit).Valence
        Case "Add Anion"
            MolecularWeight = NowProj.Anion(NumberOfIonToEdit).MolecularWeight
            Valence = NowProj.Anion(NumberOfIonToEdit).Valence
        
       Case "Edit Cation"
            MolecularWeight = NowProj.Cation(NumberOfIonToEdit).MolecularWeight
            Valence = NowProj.Cation(NumberOfIonToEdit).Valence
       Case "Add Cation"
            MolecularWeight = NowProj.Cation(NumberOfIonToEdit).MolecularWeight
            Valence = NowProj.Cation(NumberOfIonToEdit).Valence
    End Select
    If (un = "MG/L") Then x = 1#
    If (un = "µG/L") Then x = 1# * 1000
    If (un = "g/L") Then x = 1# / 1000#
    If (un = "meq/L") Then x = Valence / MolecularWeight
    If (un = "eq/L") Then x = Valence / MolecularWeight / 1000
    If (un = "mmol/L") Then x = 1# / MolecularWeight
    If (un = "µmol/L") Then x = 1000# / MolecularWeight
    If (un = "gmol/L") Then x = 1 / 1000# / MolecularWeight
  End If
  If (ut = "PRESSURE") Then
    ''If (un = "N/M²") Then X = 1#
    'If (un = "PA") Then x = 1#
  ...
  End If
  If (ut = "RESIN_CAPACITY") Then
    If (un = "MEQ/G") Then x = 1#
    If (un = "MEQ/ML BED") Then
      ColumnArea = Pi * (NowProj.Bed.Diameter * 100#) ^ 2 / 4
      x = NowProj.Bed.Weight * 1000# / ColumnArea / (NowProj.Bed.length * 100#)
    End If
    If (un = "MEQ/ML RESIN") Then x = _
...
Dim Found As Integer
  ...
  If (u = "mass_emission_rate") Then
    ...
    cbc.AddItem "mg/min"
  End If
  If (u = "resin_capacity") Then
    Found = True
    cbc.AddItem "meq/g"
    cbc.AddItem "meq/ml bed"
    cbc.AddItem "meq/ml resin"
  End If
