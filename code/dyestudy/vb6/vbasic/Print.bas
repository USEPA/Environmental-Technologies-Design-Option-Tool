...
Dim is_first As Boolean
  ...
  Call PrintCell(f1, r, 1, "Time", False, True, False)
  Call PrintCell(f1, r, 2, "Concentration", False, True, False)
  r = r + 1
  ...
  If FileExists(in_file) Then
    ...
    Select Case Not is_first
    
      Case True
        If NumArgs = 2 Then
          StrTextLine = Trim$(textline1)
          NumArgs = 1
          Call PrintCell(f1, r + 1, 1, StrTextLine, False, True, False)
        Else
          StrTextLine = Trim$(textline1)
          NumArgs = 2
          Call PrintCell(f1, r + 1, 2, StrTextLine, False, True, False)
          r = r + 1
        End If
...
Dim Any_Error As Boolean
  ...
  If (frmPrint_DO_INPUTS) Then
    ...
    in_file = App.Path & "\exes\input.txt"
    f1.SheetName(1) = "Input"
...
'    Call Print_Inputs(f1, proj, 2, in_file)
  ...
  If (frmPrint_DO_OUTPUTS) Then
      out_file = App.Path & "\exes\outpt.txt"
      f1.SheetName(2) = "TanksInSeries"
      Call Print_Outputs(f1, proj, 3, out_file)
      out_file = App.Path & "\exes\pecoutpt.txt"
      f1.SheetName(3) = "Dispersion"
      Call Print_Outputs(f1, proj, 2, out_file)
  End If
