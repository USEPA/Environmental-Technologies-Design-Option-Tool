...
C
     ...
     $          TDATA1(600),TIE1(5),TINC1(4),BR(14,14)
      ...
      COMMON/BLOCKC/XNI,CBO
      COMMON NC,NCOMP
      OPEN (3, FILE = 'COL.TXT')
      OPEN (4, FILE = 'BPSDM4.DAT')
      OPEN (7, FILE = 'BPSDM4.OUT')
C
C.....read in data from namelist
C
      READ(4,*) WT,VOL,RAD,RHOP,EPS1,EPOR,NDATA
      READ(4,*) DT0,DSTEP,DTOL,DH0,DOUT1,NM,IFER
      READ(4,*) DS,SHIFT
      READ(4,*) KF
      READ(4,*) DP
      READ(4,*) XK
...
22    CONTINUE
      ...
      IF (IFINAL .EQ. IFER) THEN
       WRITE(*,130) CBO,XK,XN,MW,KF,DS,DGS,DP,DGP,BIC
       WRITE(7,130) CBO,XK,XN,MW,KF,DS,DGS,DP,DGP,BIC
       WRITE(*,140)
       WRITE(7,140)
      ENDIF
C
...
60    ITP=ITP+1
      ...
      DOUT=TOUT/TCONV
      IF (IFINAL .EQ. IFER) THEN
       WRITE(*,150) ITP,DOUT,Y0(NC+1)*CBO*MW
       WRITE(7,150) ITP,DOUT,Y0(NC+1)*CBO*MW
      ENDIF
...
C
      ...
      IF (IFINAL .EQ. IFER) WRITE (7,170)
      CALL OBJFUN(TDATA,NDATA,ITP)
      WRITE(*,200) DS,FMIN
      WRITE(7,200) DS,FMIN
      IF (IFINAL .EQ. IFER) GOTO 65
      IF (IFINAL .EQ. 0 .OR. IFINAL .EQ. 2 .OR. IFINAL .EQ. 4
     $.OR. IFINAL .EQ. 6 .OR. IFINAL .EQ. 8) THEN
       IF (FMIN1-FMIN .LT. 0.0D0) THEN
        SHIFT=SHIFT/2.0D0
        DS=DS-SHIFT
        FMIN1=FMIN
        IFINAL=IFINAL+1
...
70     WRITE(7,190) TDATA(J),CD(J)*CBO*MW,CINT(J)*CBO*MW,RES
75    CONTINUE
85    STOP
C
C
...
130   FORMAT(/'PARAMETERS FOR COMPONENT',//,   
     $ 4X,'INITIAL BULK CONCENTRATION, CBO (MMOL/L)........ =',G15.5/   
     $ 4X,'FREUNDLICH ISO. CAP., XK (MMOL/GM)/(L/MMOL)**XN. =',G15.5/   
     $ 4X,'FREUNDLICH ISOTHERM EXPONENT, XN (DIM.)......... =',G15.5/
     $ 4X,'MOLECULAR WEIGHT, MW ........................... =',G15.5/
     $ 4X,'FILM TRANSFER COEFFICIENT, KF (CM/SEC).......... =',G15.5/   
     $ 4X,'SURFACE DIFFUSION COEFFICIENT, DS (CM**2/SEC)... =',G15.5/   
     $ 4X,'SURFACE SOLUTE DIST. PARAMETER, DGS (DIM.)...... =',G15.5/   
     $ 4X,'PORE DIFFUSION COEFFICIENT, DP (CM**2/SEC)...... =',G15.5/   
     $ 4X,'PORE SOLUTE DIST. PARAMETER, DGP (DIM.)......... =',G15.5/
     $ 4X,'SURFACE & PORE BASED BIOT NUMBER, BIC (DIM.).... =',G15.5/)  
140   FORMAT(///20X,'MODEL PREDICTION'///,'ITP',3X,'TIME(min)',3X,
     $'C/CO',/)   
150   FORMAT(1X,I3,1X,G20.10,2X,6(4X,G20.10))
160   FORMAT('MORE STEPS ATTEMPTED THAN NSTEPS',16X,
     $       'NSTEPS =',I3/,'AND TOUT(MIN)=',G10.6) 
170   FORMAT(///15X,'MODEL PREDICTION vs. DATA'//)  
180   FORMAT(/5X,'RESULTS',///,5X,'TIME(dim)',6X,'CONC(data)'
     $,5X,'CONC(pred)',6X,'RESIDUAL'/)
190   FORMAT(G16.9,2X,G16.9,2X,G16.9,2X,G16.9)   
