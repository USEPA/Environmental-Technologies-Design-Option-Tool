...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      rem= (q*co1)-(q*ca)
c      write (15,2000) rvolw1,rem
c 2000 format (' rvolw1,reminw',2e16.9)
      RETURN
      END
...
C...........CONVERT QGGC FROM L/MIN TO L/DAY
     ...
     1          *DFLOAT(NAGC)
      ...
      rem=((qagc*ca*(1.D0+(KP1*X0)))-(qagc*cb*(1.D0+(KP1*X0))))*
     1 dfloat(nagc)
      remx=rstrpg+rvolag
c      write (15,2000) remx,rem
c 2000 format (' remx,remagc',2e16.9)
...
C.....2) PRIMARY EFFLUENT WEIR DROP
      ...
      rem=((qpc*cb*(1.D0+(KP1*X0)))-(qe1*cd))*dfloat(npc)
      remx=rsorpi+rsorpw+rvolpri+rvolw2
c      write (15,2000) remx,rem
c 2000 format (' remx,rempri',2e16.9)
...
C...........CONVERT QV FROM L/MIN TO L/DAY
         ELSE
            AGA(IC)=0.D0
         ENDIF
         AT1=QAERDIV*CD/(QTDIV*(1.D0+(KP1*XBMT)))
...
C.....A SYSTEM OF N EQUATIONS, AX=B.
      CALL DECOMP(NTKP1,NTKP1,A,IPVT,DET)
      CALL SOLVE(NTKP1,NTKP1,A,IPVT,B)
...
C.....SECONDARY CLARIFIER WEIR
      ...
      rem=((qe1*cd)*dfloat(npc))-((qtdsc*tce2)*dfloat(nsc))
      remx=trbios+trsufs+trbuls+trvlsa+rsopsc+rsopsw
     1    +trvlsb+rvolw3
c      write (15,2000) remx,rem
c 2000 format (' remx,remsec',2e16.9)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      WRITE (20, *) 
      WRITE (20,2007) ' CONTAMINANT NUMBER ',NAME 
      WRITE (20, *) 
...
C is written over the b vector
      ...
      SUBROUTINE DECOMP (NDIM,N,A,IPVT,DET)
     
      IMPLICIT REAL*8 (A-M,O-Z) 
      INTEGER NDIM, N,IPVT(10)
      REAL *8 A(10,10),DET
      REAL *8 P,T
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE SOLVE (NDIM,N,A,IPVT,B)

      IMPLICIT REAL*8 (A-M,O-Z) 
      INTEGER NDIM, IPVT(10)
      REAL *8 A(10,10),B(10)
