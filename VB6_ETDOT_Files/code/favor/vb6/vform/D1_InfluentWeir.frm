...
Begin VB.Form frmD1_InfluentWeir 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Enter Parameters [Influent Weir]"
   ClientHeight    =   4680
   ClientLeft      =   6090
   ClientTop       =   1905
   ClientWidth     =   7500
   ...
   Begin VB.Label Label1 
      Caption         =   $"D1_InfluentWeir.frx":0000
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmD1_InfluentWeir"
...
Dim frmD1_InfluentWeir_Is_Dirty As Boolean
...
Const frmD1_InfluentWeir_declarations_end = True


Sub frmD1_InfluentWeir_Edit( _
    OUTPUT_Raise_Dirty_Flag As Boolean)
  Temp_Plant = NowProj.Plant
  frmD1_InfluentWeir.Show 1
  If (USER_HIT_OK) Then
...
Sub frmD1_InfluentWeir_PopulateUnits()
Dim Frm As Form
Set Frm = frmD1_InfluentWeir
  ...
  Call unitsys_register(Frm, lblData(0), txtData(0), cboUnits(0), "length", _
      Temp_Plant.InfluentWeir.UnitsOfDisplay(0), "m", "", "", 100#, True)
  Call unitsys_register(Frm, lblData(1), txtData(1), cboUnits(1), "length", _
      Temp_Plant.InfluentWeir.UnitsOfDisplay(1), "m", "", "", 100#, True)
  ''''Call unitsys_register(Frm, lblData(2), txtData(2), cboUnits(2), "flow_volumetric_per_length", _
      Temp_Plant.InfluentWeir.UnitsOfDisplay(2), "m³/m-h", "", "", 100#, True)
  Call unitsys_register(Frm, lblData(2), txtData(2), cboUnits(2), "flow_volumetric_per_length", _
      Temp_Plant.InfluentWeir.UnitsOfDisplay(2), "m³/(m-h)", "", "", 100#, True)
End Sub
...
Set Ctl = cbo_Model_Type
  ...
  Temp_Plant.InfluentWeir.ModelingMechanism = Ctl.ItemData(Ctl.ListIndex)
  'RAISE DIRTY FLAG AND REFRESH WINDOW.
  Call Local_DirtyStatus_Set(frmD1_InfluentWeir_Is_Dirty, True)
  Call frmD1_InfluentWeir_Refresh(Temp_Plant)
End Sub
...
Private Sub Form_Load()
  ...
  Call Local_DirtyStatus_Set(frmD1_InfluentWeir_Is_Dirty, False)
  ...
  '
  Call frmD1_InfluentWeir_PopulateUnits
  '
  ' REFRESH DISPLAY.
  '
  Call frmD1_InfluentWeir_Refresh(Temp_Plant)
  ...
  If (NewValue_Okay) Then
    If (Raise_Dirty_Flag) Then
      ...
      Select Case Index
        ...
        ' MAIN DATA BLOCK.
        '
        Case 0: Temp_Plant.InfluentWeir.Width = NewValue
        Case 1: Temp_Plant.InfluentWeir.WaterLevelDiff = NewValue
        Case 2: Temp_Plant.InfluentWeir.GasFlow = NewValue
      End Select
      If (Raise_Dirty_Flag) Then
        'THROW DIRTY FLAG.
        Call Local_DirtyStatus_Set(frmD1_InfluentWeir_Is_Dirty, True)
