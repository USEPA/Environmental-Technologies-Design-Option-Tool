\input vanilla.sty          %input standard vanilla style
\input stdmath.tex          %input ctm math macros
\hfuzz=1pt                  %overfull box to 1 pt is ok
\magnification=\magstep1    %magnify by a factor of 1.2
\baselineskip=12 pt         %single
%\nopagenumbers              %do not paginate letter
\parindent=0 pt             %set paragraph indent to flat format
\parskip= 5 pt
...
\underbar{General Description of VOC Modeling and Unit Processes}
...
Each type of unit process is treated as a reactor in series with the other unit
...
\underbar{Volatilization}

Volatilization occurs by way of air flow across the AGC surface. In an open
...
Sorption is modeled as an equilibrium process; thus, for low aqueous
...
\mb
\log K_p \; \text{(liters/gm VSS)} = 0.58 \log K_{ow} + 1.14
\mef
or that of Matter-M\"uller {\it et al.} (1980)
\mb
\log K_p \; \text{(liters/gm TS)} = 0.67 \log K_{ow} - 2.61
\mef
...
\underbar{Stripping - Surface Aeration}
...
The overall mass transfer coefficient for surface aeration is based on
...
\underbar{Stripping - Diffused Bubble Aeration}
...
\item{-} whether (a) the user will enter or (b) FaVOr will calculate biomass
...
The user must specify the following parameters in the Biomass Calculations
screen:
...
SECONDARY CLARIFIER WEIR SCREEN
