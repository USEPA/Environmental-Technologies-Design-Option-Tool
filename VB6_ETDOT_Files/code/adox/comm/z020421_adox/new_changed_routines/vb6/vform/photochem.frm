...
Begin VB.Form frmPhotoChem 
   ...
   Caption         =   "Editing Photochemical Reactor Properties"
   ClientHeight    =   6600
   ClientLeft      =   3150
   ClientTop       =   1725
   ClientWidth     =   9480
   ...
   Begin Threed.SSFrame SSFrame2 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Begin VB.CommandButton cmdH2O2ExtinCoeffTable 
         Caption         =   "View Table of H2O2 Extinction Coefficients"
         Height          =   375
         Left            =   240
         TabIndex        =   24
         Top             =   870
...
Dim intUvi() As Double
Dim intWaveLength() As Double
Dim intVolume As Double
Dim intLampPower As Double
Dim i As Integer


  If (cboLightSpecMethod.ListIndex <> val(cboLightSpecMethod.Tag)) Then
    ReDim intUvi(1 To TempProj.Wavelength_Count)
    ReDim intWaveLength(1 To TempProj.Wavelength_Count)
    For i = 1 To TempProj.Wavelength_Count
      intUvi(i) = TempProj.Wavelengths(i).uvi
      intWaveLength(i) = TempProj.Wavelengths(i).lwave
    Next i
    intVolume = TempProj.volume
    intLampPower = TempProj.lamp_power
    
''''''0 - Light intensity in (eins/L*s)
''''''1 - Light intensity in (watts)
''''''2 - UV efficeincy
''''''
''''''Equation for changing from 0-1:
''''''UV(watts) = UV(eins / L * s) * 0.12 * volume(L)* 1E9 / Wavelength(nm)
''''''Equation for changing from 0-2:
''''''UV efficiency = UV(eins / L * s) * 0.12 * volume(L)*1E9 / [Wavelength(nm)*Lamp Power(watts)]
''''''Equation for changing from 1-0:
''''''UV(eins / L * s) = UV(watts) * Wavelength(nm)* 1E-9 / [0.12*Volume(L)]
''''''Equation for changing from 1-2:
''''''UV efficiency = UV(watts)/Lamp Power(watts)
''''''Equation for changing from 2-0:
''''''UV(eins/L*s) = UV efficiency * Lamp Power(watts) * Wavelength(nm)*1E-9 /[0.12*Volume(L)]
''''''Equation for changing from 2-1:
''''''UV (watts) = UV efficiency * Lamp Power(watts)

  Select Case val(cboLightSpecMethod.Tag)
      Case 0
        Select Case cboLightSpecMethod.ItemData(cboLightSpecMethod.ListIndex)
          Case IDUVI_WATTS:
            TempProj.iduvi = IDUVI_WATTS
            For i = 1 To TempProj.Wavelength_Count
              TempProj.Wavelengths(i).uvi = _
                intUvi(i) * 0.12 * intVolume * 1000000000# / intWaveLength(i)
            Next i
          Case IDUVI_EFFICIENCY:
            TempProj.iduvi = IDUVI_EFFICIENCY
            For i = 1 To TempProj.Wavelength_Count
              TempProj.Wavelengths(i).uvi = _
                intUvi(i) * 0.12 * intVolume * 1000000000# / (intWaveLength(i) * intLampPower)
            Next i
        End Select
      Case 1
        Select Case cboLightSpecMethod.ItemData(cboLightSpecMethod.ListIndex)
          Case IDUVI_EINSTEINS_L_S:
            TempProj.iduvi = IDUVI_EINSTEINS_L_S
            For i = 1 To TempProj.Wavelength_Count
              TempProj.Wavelengths(i).uvi = _
                intUvi(i) * intWaveLength(i) * 0.000000001 / (0.12 * intVolume)
            Next i
          Case IDUVI_EFFICIENCY:
            TempProj.iduvi = IDUVI_EFFICIENCY
            For i = 1 To TempProj.Wavelength_Count
              TempProj.Wavelengths(i).uvi = _
                intUvi(i) / intLampPower
            Next i
        End Select
      Case 2
        Select Case cboLightSpecMethod.ItemData(cboLightSpecMethod.ListIndex)
          Case IDUVI_EINSTEINS_L_S:
            ...
            For i = 1 To TempProj.Wavelength_Count
              TempProj.Wavelengths(i).uvi = _
                intUvi(i) * intLampPower * intWaveLength(i) * 0.000000001 / (0.12 * intVolume)
            Next i
          Case IDUVI_WATTS:
            TempProj.iduvi = IDUVI_WATTS
            For i = 1 To TempProj.Wavelength_Count
...
Private Sub cmdH2O2ExtinCoeffTable_Click()
frmH2O2ExtinctionCoeffTable.Show 1
End Sub
...
Set txtctl = txtData(Index)
  ...
  If (Index = 4) Then
    ...
    Val_High = 1E+20 * 60#
  Else
    Val_Low = 0     '0.00000000000000000000
