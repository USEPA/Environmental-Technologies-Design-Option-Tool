...
Begin VB.Form frmMain 
   ...
   Begin Threed.SSFrame SSFrame6 
      ...
      _ExtentY        =   1455
      _StockProps     =   14
      Caption         =   "Photochemical Reactor Properties:"
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   0
   ...
   Begin VB.Menu cmdEdit 
      ...
      Begin VB.Menu mnuEditItem 
         Caption         =   "Edit &Dye Study"
         Index           =   10
      End
      Begin VB.Menu mnuEditItem 
         Caption         =   "Edit &Photochemical Reactor Properties"
         Index           =   20
      End
      Begin VB.Menu mnuEditItem 
         Caption         =   "&NOM and Target Compounds"
         Index           =   30
         Begin VB.Menu mnuEditNOMItem 
            Caption         =   "&Add"
            Index           =   10
         End
         Begin VB.Menu mnuEditNOMItem 
...
Private Sub cmdDyeStudy_Click()
Dim RetVal As Double
Dim fn_this As String

  'need to determine a location for dyestudy that will be consistent
  ' for all situations
  If MAIN_APP_PATH = "X:\etdot10\code\adox\vb6" Then
    fn_this = "X:\etdot10\code\dyestudy\vb6\dyestudy.exe"
  Else
    fn_this = MAIN_APP_PATH & "\..\dyestudy\dyestudy.exe"
  End If
  If (Not FileExists(fn_this)) Then
    Call Show_Message("DyeStudy application is not available.  Please contact vendor.", _
      vbExclamation, App.title)
  Else
    If UCase(get_program_releasetype()) = "BETA" Then
       RetVal = Shell(fn_this + " -fromadox_beta", 1)
    Else
      RetVal = Shell(fn_this + " -fromadox", 1)
    End If
  End If
...
Private Sub Form_Load()
  ...
  If UCase(get_program_releasetype()) = "BETA" Then
    frmMain.mnuFileItem(30).Enabled = False
    frmMain.mnuFileItem(40).Enabled = False
  End If
...
Select Case Index
  Case 10:    'Dye Study
    Call cmdDyeStudy_Click
    
  Case 20:    'Photochemical
    Call cmdPhotochemical_Click(0)
    
End Select

End Sub

Private Sub mnuEditNOMItem_Click(Index As Integer)
Select Case Index
  Case 10:    'Add
    Call cmdTarget_Click(0)
  Case 20:    'Delete
    Call cmdTarget_Click(1)
  Case 30:
    Call cmdTarget_Click(2)
  Case 40:
    Call cmdTarget_Click(3)
End Select
End Sub
...
Dim fn_this As String
  Select Case Index
    ...
    Case 20:      'ONLINE MANUAL.
      fn_this = MAIN_APP_PATH & "\help\adox_man.doc"
...
'      Call LaunchFile_General("", fn_this)
    ...
    Case 90:    'VIEW ACKNOWLEDGMENTS.
      MsgBox "The authors are grateful to Michigan Tech Masters Graduate Shumin Hu, former Michigan Tech Research Engineer Eric J. Oman, and former Michigan Tech Computer Programmer Karen A. Mansfeldt for their contributions toward the development of this product."
    Case 99:    'ABOUT.
      frmAbout.Show 1
  End Select
End Sub
...
Dim msg As String
Dim RetVal As Integer
Dim BetaReturn As Boolean

  BetaReturn = True
  If UCase(get_program_releasetype()) = "BETA" Then
    'look at values for volume, retention time and DBCP
    'initial concentration to see if they match the
    'global constants assigned:
    'BETA_VOLUME, BETA_RETENTION, BETA_DBCP_CONCINI
    'BetaReturn = False
    'also look at dirty flag
    If (NowProj.volume < BETA_VOLUME - 1 Or NowProj.volume > BETA_VOLUME + 1) Or _
      (NowProj.tau < BETA_RETENTION - 0.01 Or NowProj.tau > BETA_RETENTION + 0.01) Or _
      NowProj.TargetCompounds(2).concini < BETA_DBCP_CONCINI - 0.00000001 Or _
      BETA_DBCP_CONCINI + 0.00000001 Or _
      NowProj.dirty = True Then
      'Or Raise_Dirty_Flag = True
      BetaReturn = False
    End If
    
    'if these don't match, put up message that example has
    'been changed so the simulation cannot be run
    If (BetaReturn = False) Then
      msg = "Changes have been made to the example data file, the simulation can not be run."
      RetVal = MsgBox(msg, vbCritical, App.title)
      Exit Sub
    End If
  End If
  Select Case Index
...
Dim Too_Small As Integer
  ...
  If (Index = 4) Then
    ...
    Val_High = 1E+20 * 60#
  Else
    Val_Low = 0     '0.00000000000000000001
