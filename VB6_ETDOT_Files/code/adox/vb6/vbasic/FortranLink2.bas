...
''''BEGIN MODIFICATION 1: 18apr2002 BY DAVID R. HOKANSON
''''    fp.extcoef(i) = EXTCOEF_DEFAULT_VALUE   '' 19#  Changed by KAM per Luke
    fp.extcoef(i) = NowProj.extcoef_h2o2(i)
''''END MODIFICATION 1: 18apr2002 BY DAVID R. HOKANSON
  Next i
  ReDim fp.quatyd(1 To NowProj.nwvlen)
  For i = 1 To NowProj.nwvlen
''''BEGIN MODIFICATION 2: 18apr2002 BY DAVID R. HOKANSON
''''    fp.quatyd(i) = QUATYD_DEFAULT_VALUE     '' 0.5  Changed by KAM per Luke
    fp.quatyd(i) = NowProj.quatyd_h2o2(i)
''''END MODIFICATION 1: 18apr2002 BY DAVID R. HOKANSON
  Next i
  ...
  ReDim fp.extcoef(1 To NowProj.nwvlen)
  For i = 1 To NowProj.nwvlen
''''BEGIN MODIFICATION 3: 18apr2002 BY DAVID R. HOKANSON
''''    fp.extcoef(i) = EXTCOEF_DEFAULT_VALUE   ''0.0867  Changed by KAM per Luke
    fp.extcoef(i) = NowProj.extcoef(1, i)
''''END MODIFICATION 3: 18apr2002 BY DAVID R. HOKANSON
  Next i
  ReDim fp.quatyd(1 To NowProj.nwvlen)
  For i = 1 To NowProj.nwvlen
''''BEGIN MODIFICATION 4: 18apr2002 BY DAVID R. HOKANSON
''''    fp.quatyd(i) = QUATYD_DEFAULT_VALUE     ''0# changed by KAM per Luke
    fp.quatyd(i) = NowProj.quatyd(1, i)
''''END MODIFICATION 4: 18apr2002 BY DAVID R. HOKANSON
  Next i
  NowProj.Fortran_PhotRxn(idx + 0) = fp
  idx = idx + 1
