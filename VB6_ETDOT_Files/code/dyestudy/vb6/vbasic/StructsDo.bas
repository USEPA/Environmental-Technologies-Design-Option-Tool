...
Dim i As Integer
  ...
  'DYESTUDY PARAMETERS
  p.dyestudy_output = ""
  ReDim p.DyeStudy(1 To 1600)
  p.dyestudy_count = 1600
  ...
  Next i
  p.dyestudy_output = ""
  p.dyestudydisp_output = ""
  p.dyestudy_calcdate = ""
  p.Predicted_Available = False
  p.Predicted_count = 0
  p.Experimental_count = 0
  p.PredictedDispClosed_count = 0
  p.PredictedDispOpen_count = 0
  p.plottype = 0
  ...
  Call Kill_If_It_Exists(App.Path & "\exes\pecoutpt.txt")
  Call Kill_If_It_Exists(App.Path & "\exes\outpt4.txt")
  Call Kill_If_It_Exists(App.Path & "\exes\outpt5.txt")
End Sub
