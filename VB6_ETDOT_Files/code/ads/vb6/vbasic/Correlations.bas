...
Dim EAB As Double, E As Double, FE As Double, DiffgT As Double
  ...
  DiffgT = _
      ( _
          0.0001 * (1.084 - 0.249 * Sqr(1# / WTAIR + 1# / Component(i).MW)) * _
          (Bed.Temperature + 273.15) ^ 1.5 * Sqr(1# / WTAIR + 1# / Component(i).MW) _
      ) _
      / (Press * RAB ^ 2 * FE)
...
Dim D As Double
  ...
  Else
    ...
    If (USE_GASPHASE_WAKAO_AND_FUNAZUKRI) Then
      ...
      kf = _
          Diffg(i) / _
          (2# * Carbon.ParticleRadius * 100#) * _
          Carbon.ShapeFactor * _
          (2# + 1.1 * Re ^ 0.6 * ss ^ (1# / 3#))
      '---- End of Old code
    Else
      ...
      kf = _
          (1# + 1.5 * (1# - EBED)) * _
          Carbon.ShapeFactor * Diffg(i) / _
          (2# * Carbon.ParticleRadius * 100#) * _
          (2# + 0.644 * Re ^ 0.5 * ss ^ (1# / 3#))
    End If
  End If
  Exit Function
Error_KF:
