...
Dim un As String
  ...
  If (ut = "DENSITY") Then
    ...
    If (un = "KG/M³") Then X = 1#
    If (un = "KG/M3") Then X = 1#
    If (un = "LB/FT³") Then X = 1# * (0.45359237) / (0.028316847)
    If (un = "LB/FT3") Then X = 1# * (0.45359237) / (0.028316847)
    If (un = "LB/GAL") Then X = 1# * (0.45359237) / (0.00378541178)
  End If
  If (ut = "MOLAR_DENSITY") Then
    If (un = "KMOL/M3") Then X = 1#
    If (un = "KMOL/M³") Then X = 1#
    If (un = "GMOL/M³") Then X = 1# * 0.001
  End If
...
''''    If (un = "KG/KMOL") Then X = 1#
  ...
  If (ut = "MASS_EMISSION_RATE") Then
    ...
    'If (un = "MG/MIN") Then X = 1000# / 60#
  End If
  If (ut = "MOLAR_ENERGY") Then
    If (un = "J/MOL") Then X = 1#
    If (un = "J/GMOL") Then X = 1#
    If (un = "J/KMOL") Then X = 1# / 1000#
  End If
  If (ut = "MOLAR_THERMAL_ENERGY") Then
    If (un = "J/MOL-K") Then X = 1#
    If (un = "J/GMOL-K") Then X = 1#
    If (un = "J/KMOL-K") Then X = 1# / 1000#
    If (un = "J/KMOL·K") Then X = 1# / 1000#
  End If
  If (ut = "DIFFUSIVITY") Then
    If (un = "CM²/S") Then X = 1#
    If (un = "CM2/S") Then X = 1#
    If (un = "M²/S") Then X = 1# * 100# * 100#
    If (un = "M2/S") Then X = 1# * 100# * 100#
  End If
  If (ut = "SURFACE_TENSION") Then
    If (un = "N/M") Then X = 1#
    If (un = "N/CM") Then X = 1# * 100#
  End If
  If (ut = "ACTIVITY_COEFFICIENT") Then
    If (un = "UNIT-LESS") Then X = 1#
  End If
  If (ut = "THERMAL_CONDUCTIVITY") Then
    If (un = "W/M/K") Then X = 1#
    If (un = "W/M·K") Then X = 1#
    If (un = "W/CM/K") Then X = 1# * 100#
  End If
  If (ut = "PERCENT_VOLUME") Then
    If (un = "VOL% IN AIR") Then X = 1#
  End If
  If (ut = "OXYGEN_DEMAND") Then
    If (un = "G O2/G CHEM") Then X = 1#
  End If
  If (ut = "LOG_KOW") Then
    If (un = "UNIT-LESS") Then X = 1#
    If (un = "LOG KOW") Then X = 1#
  End If
  If (ut = "LOG_KOC") Then
    If (un = "CM3/G OC") Then X = 1#
    If (un = "CM3/G OM") Then X = 1#
  End If
  If (ut = "BIOCONC_FACTOR") Then
    If (un = "UNIT-LESS") Then X = 1#
  End If
...
Dim H As Integer
  Found = False
  For i = 1 To UBound(unitsys)        'lbound(unitsys)
      'unitsys(i).lblx.caption
    If (Not unitsys(i).deleted) Then
'txtx.hwnd
'unitsys(i).txtx.hWnd
'unitsys(i).txtx.name
...
Dim Found As Integer
  ...
  If (u = "mass_emission_rate") Then
    ...
    cbc.AddItem "mg/min"
  End If
  If (u = "molar_energy") Then
    Found = True
    cbc.AddItem "J/mol"
    cbc.AddItem "J/gmol"
    cbc.AddItem "J/kmol"
  End If
  If (u = "molar_thermal_energy") Then
    Found = True
    cbc.AddItem "J/mol-K"
    cbc.AddItem "J/gmol-K"
    cbc.AddItem "J/kmol-K"
    cbc.AddItem "J/kmol·K"
  End If
  If (u = "diffusivity") Then
    ...
    cbc.AddItem "m2/s"
  End If
  If (u = "surface_tension") Then
    Found = True
    cbc.AddItem "N/m"
    cbc.AddItem "N/cm"
  End If
  If (u = "activity_coefficient") Then
    Found = True
    cbc.AddItem "unit-less"
  End If
  If (u = "thermal_conductivity") Then
    Found = True
    cbc.AddItem "W/m/K"
    cbc.AddItem "W/m·K"
    cbc.AddItem "W/cm/K"
  End If
  If (u = "percent_volume") Then
    Found = True
    cbc.AddItem "vol% in air"
  End If
  If (u = "oxygen_demand") Then
    Found = True
    cbc.AddItem "g O2/g chem"
  End If
  If (u = "log_kow") Then
    Found = True
    cbc.AddItem "unit-less"
  ...
  If (u = "log_koc") Then
    Found = True
    cbc.AddItem "cm3/g OC"
    cbc.AddItem "cm3/g OM"
  End If
  If (u = "bioconc_factor") Then
    Found = True
    cbc.AddItem "unit-less"
  End If
...
Dim max As Integer
  Found = False
  max = CboX.ListCount - 1
  For i = 0 To max
    If (Trim$(UCase$(CboX.List(i))) = Trim$(UCase$(new_units))) Then
      Found = True
      Exit For
    End If
  Next i
...
Dim max As Integer
  ...
  If (H <= 0) Then Exit Sub
  Found = False
  Call unitsys_set_units0(unitsys(H).CboX, new_units)
End Sub
