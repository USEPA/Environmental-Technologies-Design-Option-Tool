...
Sub file_new()
  '
  ' INITIALIZE FOR DEFAULT VARIABLE SETTINGS.
  '
  ...
  '
  Call unitsys_set_units(frmMain.txtdata(0), NowProj.Op_T_UnitDisplayed)
  Call unitsys_set_units(frmMain.txtdata(1), NowProj.Op_P_UnitDisplayed)
  '
...
Dim DataVersion_Minor As Integer
  ...
  If (ShowLegacyWarning) Then
    Call Show_Message00("Warning: This file is formatted as a " & _
        ...
        " file.", _
        vbInformation, _
        Name_App_Short & " : Legacy File Version Warning")
  End If
  Close #F
  'UPDATE DISPLAY.
...
Dim msg As String
  ...
  End If
  msg = msg & " ?"
  RetVal = MsgBox(msg, vbCritical + vbYesNoCancel, Name_App_Short & " : Save Changes ?")
  Select Case RetVal
    Case vbYes:
